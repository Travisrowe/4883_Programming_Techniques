...
int main()
{
	...
	while (cin >> a)
	{
		...
		long double dottedArea = (triangleDEFArea + DEFOuterSlivers) * 4;
		...
		long double gridArea = (triangleDFCArea - 2 * DEFOuterSlivers) * 4;

		printf("%.3f %.3f %.3f\n", centerArea, dottedArea, gridArea);
	}
}